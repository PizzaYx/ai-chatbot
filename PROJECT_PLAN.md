...
> **版本**: v3.2 (Digital Human + TTS) | **最后更新**: 2026-01-26
...
- [x] **流式响应 (Streamable HTTP)**: 实现打字机效果，采用 NDJSON 协议，支持中途打断。d
...
#### Phase 2: 交互增强 (优先级: 高) 🆕
- [x] **🎤 语音输入 (Speech-to-Text)**: ✅ 已完成
    - **方案**: FunASR（阿里开源语音识别）
      - 费用：免费开源
      - 部署：NAS Docker 容器
      - 镜像：`registry.cn-hangzhou.aliyuncs.com/funasr_repo/funasr:funasr-runtime-sdk-online-cpu-0.1.13`
      - 端口：10095（WebSocket）
      - 模型：Paraformer + VAD + 标点恢复
    - **后端接口**: `POST /api/stt/recognize`（Django）
    - **前端交互**: 
      - 点击麦克风按钮开始录音
      - 微信风格全屏录音界面（声波动画 + 时长显示）
      - 录音结束后自动识别转文字
      - 支持 MediaRecorder API 录音 + PCM 格式转换
- [x] **🔊 语音合成 (Text-to-Speech)**: ✅ 已完成
    - **方案**: Edge-TTS（微软神经网络语音）
      - 费用：免费，无需 API Key
      - 音质：极好（支持 `zh-CN-XiaoxiaoNeural` 等高品质声音）
      - 延迟：0.3-1秒
    - **后端接口**: `/api/tts/speak` 返回 MP3 音频流
    - **前端功能**: 播放/停止按钮、音频缓存
    - **🔜 后期优化**: 流式播放（边生成边播放，需 MediaSource API）
- [x] **数字人交互 (Digital Human)**: ✅ 已完成
    - **技术方案**: Ready Player Me 3D Avatar + Three.js
    - **模型来源**: Ready Player Me (GLB 格式，支持本地/远程加载)
    - **渲染引擎**: Three.js (WebGL)
    - **驱动核心**:前端实时音频频域分析 (Web Audio API Analyser)
    - **口型同步**: 自研实时音量驱动算法 (Real-time Volume to Viseme Mapping)。不依赖服务器端音素分析，零延迟，支持口型 (aa/O) 自动混合与下巴 (`jawOpen`) 联动。
    - **全脸动效**: 基于音量的多维度面部 Morph Target 联动（眉毛、眨眼、皱鼻、脸颊鼓起、视线游移）。
    - **骨骼律动**: 程序化骨骼动画 (Head/Neck/Spine)，实现自然的待机呼吸感与说话时的点头/摇晃动作。
    - **性能**: 纯前端运行，极低资源占用。
- [x] **🔄 消息重新生成**: ✅ 已完成
    - AI 回复旁有刷新按钮，点击重新生成回复
- [x] **✏️ 消息编辑**: ✅ 已完成
    - 用户消息支持点击编辑
    - 编辑后重新发送并更新对话上下文
 ...
 *   **2026-02-02 (v3.5)**: 完成数字人 (Digital Human) 深度交互。基于 Three.js + Ready Player Me 实现前端纯音频驱动的 3D 虚拟人。突破性地实现了"Viseme 混合 + 骨骼程序化律动"算法，无需后端参与即可实现自然的口型同步（混入 aa/O 音素）、全脸微表情（眉毛/鼻子/眨眼）及头部/身体的自然呼吸感与跟随动作。支持本地 GLB 模型加载，解决了网络延迟问题。
 *   **2026-01-30 (v3.4)**: 完成语音输入 STT 功能。采用 FunASR（阿里开源）部署在 NAS Docker，通过 frp 穿透访问。前端实现微信风格录音界面（全屏遮罩 + 声波动画 + 时长显示），WebM 录音转 PCM 格式后调用后端识别。
*   **2026-01-26 (v3.3)**: 完成语音合成 TTS 功能。采用 Edge-TTS（微软神经网络语音），实现播放/停止/缓存功能。规划数字人方案：Ready Player Me + Three.js + Wawa-Lipsync。
